...
class TorchserveGrpcManager(TorchserveBaseManager):
    ...
    def __init__(
        ...
        Args:
            inference_port (str): Inference port, where GRPC runs. Defaults to "7443".
            management_port (str): Management port where GRPC torchserve may be controlled.
                Defaults to "7444".
    ...
    @error_wrapper(error_return_value=None)
    def _infer_without_worker_check(self, model_name: str, image: str | np.ndarray, **kwargs):
        ...
        if isinstance(image, str):
            ...
            data = cv2.imencode("{}".format(self.image_encoder), image)[1].tobytes()
    ...
    def infer(self, model_name: str, image: str | np.ndarray, **kwargs):
        ...
        Returns:
            Dict: Processed data from the model
        """
        inference_output = self._infer_without_worker_check(model_name, image, **kwargs)
